...
public abstract class BaseJCacheTester {
    ...
    protected <K, V> boolean simpleGetsAndPutsCacheTest(Cache<K, V> myJCache, int numberOfIteration, int numberOfObjectPerIteration, int sleepTimeMillisBetweenIterations,
            KeyValueGenerator<K, V> keyValueGenerator) throws InterruptedException {
        try {
            ...
            if (null != myJCache) {
                ...
                while (iterationCount < numberOfIteration) {
                    ...
                    LOGGER.info("simpleGetsAndPutsCacheTest - Done Iteration #{} of {} in {} micros - total cache ops: {} hits: {} / misses: {}", iterationCount + 1, numberOfIteration,
                            (duration / 1000), opsCount, hitCounts, missCount);
    ...
    private <K, V> void inspectCacheConfig(Cache<K, V> myJCache) {
        ...
        CacheRuntimeConfiguration<K, V> ehcacheConfig = (CacheRuntimeConfiguration<K, V>) myJCache.getConfiguration(Eh107Configuration.class)//
