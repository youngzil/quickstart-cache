...
#启动新节点实例：
redis-5.0.8/src/redis-server 7006/redis.conf
redis-5.0.8/src/redis-server 7007/redis.conf
...
redis-5.0.8/src/redis-cli --cluster add-node 127.0.0.1:7006 127.0.0.1:7000
...
redis-5.0.8/src/redis-cli --cluster reshard 127.0.0.1:7000
...
master_id=`redis-5.0.8/src/redis-cli -p 7000 cluster nodes | grep master | grep 7006 | awk '{print $1}'`
echo $master_id
redis-5.0.8/src/redis-cli --cluster add-node 127.0.0.1:7007 127.0.0.1:7000 --cluster-slave --cluster-master-id $master_id
